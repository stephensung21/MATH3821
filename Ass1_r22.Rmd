...
d) 
  ...
  \
  We note that $\sum_{i=1}^{n} (y_i-\bar{y})(x_i-\bar{x})=\sum_{i=1}^{n} (x_i-\bar{x})y_i$
...
e) 
```{r}
curve(exp(1+x)/(1+exp(1+x)), xlim = c(-5, 5), ylim = c(0, 1), main=expression(paste("Sampled values, ", mu, "=5, ", sigma,
    "=1")))
```
